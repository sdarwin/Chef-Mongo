...
#S.D. 2012 , fixing a bug
Chef::Log.info("members before the unless #{members}")
...
if tester == "ok"
 ...
 Chef::Log.info("members after the unless #{members}")
	...
	#S.D.2012 debug
	debugger=*old_members.collect{ |m| m.split(":") }
        Chef::Log.warn("Debug1 'old_members is #{old_members} and the collect is #{debugger}'")
        ...
        #S.D.2012 debug
        debugger=*old_members.collect{ |m| m.split(":") }
        Chef::Log.warn("Debug2 'old_members is #{old_members} and the collect is `#{debugger}'")
        #rs_connection = Mongo::ReplSetConnection.new( *old_members.collect{ |m| m.split(":") })
        rs_connection = Mongo::ReplSetConnection.new( old_members )
        admin = rs_connection['admin']
  ...
  def self.configure_shards(node, shard_nodes)
    ...
    begin
      #S.D. 2012
      connection = Mongo::Connection.new('localhost', node['mongodb']['mongos']['port'], :op_timeout => 5)
    rescue Exception => e
      #S.D. 2012
      Chef::Log.warn("Could not connect to database: 'localhost:#{node['mongodb']['mongos']['port']}', reason #{e}")
      return
    end
