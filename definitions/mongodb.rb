...
define :mongodb_instance, :mongodb_type => "mongod" , :action => [:enable, :start], :port => 27017 , \
    ...
    :configserver => [], :replicaset => nil,  :enable_rest => false, :notifies => [] do
  ...
  if type != "mongos"
    ...
    configfile = nil
    dbpath = nil
    configserver = configserver_nodes.collect{|n| "#{n['fqdn']}:#{n['mongodb']['configserver']['port']}" }.join(",")
  end
  ...
  template "#{node['mongodb']['defaults_dir']}/#{name}" do
    ...
    variables(
      ...
      #obsolete, and probably never worked anyway
...
template "#{node['mongodb']['init_dir']}/#{name}" do
    ...
    owner "root"
    mode "0755"
    variables :provides => name, :initport => port
  ...
  service name do
    ...
    if name == "mongodb"
      ...
      ignore_failure true
    	...
    	#S.D. 2012
  	#on bootstrap, this recipe fails often, due to chicken-and-egg issues and must be run twice
	#so, ignore failures
     	ignore_failure true
  end
